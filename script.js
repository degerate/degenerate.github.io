...
const scale = 25; // Each unit will represent 25 pixels on the canvas for -10 to 10 range.
let shape = [
    { x: 100, y: 100 },
    { x: 150, y: 100 },
    { x: 150, y: 150 },
...
function drawShape() {
    ...
    shape.forEach((point, index) => {
        const x = origin.x + (point.x / 10) * scale; // Adjusted for -10 to 10 range
        const y = origin.y - (point.y / 10) * scale;
        if (index === 0) {
            context.moveTo(x, y);
        } else {
            context.lineTo(x, y);
...
function updateCenterCoordinates() {
    ...
    coordinatesDisplay.innerHTML = `Coordinates: (${Math.round(centerX)}, ${Math.round(centerY)})`;
}
